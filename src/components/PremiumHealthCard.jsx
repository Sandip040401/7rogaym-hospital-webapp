    ...
    <div className='h-60 w-96 border-2 overflow-hidden' id={`card-${member?._id || 'null'}`}>
      ...
      <div className='relative'>
        ...
        {member?.photo ? (
          ...
          <div className='h-24 w-20 absolute border-2 border-black' style={{ left: '30px', bottom: '-70px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <span>No Photo</span>
          </div>
        )}
        <div className='absolute max-w-36' style={{ left: '125px', bottom: '-80px' }}>
          <div>
            <p className='text-custom font-medium text-slate-500'>Name</p>
            <p className='text-custom1 font-medium'>{capitalizeFirstLetter(member?.name)}</p>
          </div>
          <div>
        ...
        <div className='absolute' style={{ left: '250px' }}>
          ...
          <p className='text-custom1 font-medium'>{`${capitalizeFirstLetter(member?.gender)}/${member?.age || 'null'} Yrs`}</p>
        </div>
